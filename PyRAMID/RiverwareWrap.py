...
class RiverwareWrap(object):
    def __init__(self, WD, copyFromRootForGA_RootPath = None, \
        ...
        if copyFromRootForGA_RootPath is not None:
            ...
            assert os.path.isdir(copyFromRootForGA_RootPath), \
                self.logger.error("PathError The copyFromRootForGA_RootPath "+\
                                  "is not exist {}"\
                                      .format(copyFromRootForGA_RootPath))
        ...
        if copyFromRootForGA_RootPath is not None:
            self.copyFolder(src = os.path.join(copyFromRootForGA_RootPath, \
                                               "ABM"), \
        ...
        self.logger.info("Subfolders' path:\n"+"\n".join(['{:^23} :  {}'\
                .format(keys, values) for keys,values in self.PATH.items()]))

        # Folder deletion lock => reduceStorageOfGivenFolder() can only be 
        # executed when it is a copied folder.
        ...
        try: 
            ...
            with open(ArchiveFilePath, "rb") as f:
                self.ArchiveFiles = pickle.load(f)  # Path of archive files.
    ...
    def copyFolder(self, src, dest, overwrite = False):
        '''
        Cpoy the following src folder to dest. Please make sure the dest 
        folder is not exist.
    ...
    def reduceStorageOfGivenFolder(self, reduceLevel = "keep_result_only"):
        '''
        Be careful when using this function. This function will delete the 
        whole folder.
    ...
    def resetDMIPath2mdlFile(self):
        ...
        # r+: read and write
        f = open(os.path.join(self.PATH["RWModel_Path"], findmdl[0]), "r+") 
        # Read
        RWFile = f.read()
    ...
    def createYeartxt(self, StartYear, EndYear, ABMoffsetYear = 2):
        ...
        # According to our algorithm, we need at least a year record to run 
        # the algorithm.
    ...
    def addYear2Yeartxt(self):
        # r+: read and write
        f = open(os.path.join(self.PATH["ABM_Path"], "year.txt"), "r+") 
        # Read
        y = int(f.readline()) # current year
    ...
    def writeRWInputFile(self, ObjSlotName, Data, Head, \
        ...
        ----------
        ObjSlotName : string
            The ObjSlotName is the filename, which should consist to the name 
            in control files.
        Data : list
            List of the data, which the length should consist with the date 
            length in "Head".
        Head : string
            Pre-defined header for the input data file. The header is save in 
            self.Header[<Head>].
        DataFolder : string
    ...
    def createFiles(self, FileContentDict, ABMpyFilename = "ABM.py"):
        ...
        f.write("python %~dp0{} {}".format(ABMpyFilename, self.WD))  
        f.close()
        self.logger.info("{} is created at {}"\
    ...
    def readRWOutputFile(self, Filename, DataFolder = "RW_Output_toABM", \
        ...
        if "start_date" in text_file[0]:
            ...
            start_date = [i for i in text_file[0:10] if "start_date" in i][0]\
                          .split(" ")[1].replace("-", "/")
            
        if "end_date" in text_file[1]:
            end_date = text_file[1].split(" ")[1]
        else:
            end_date = [i for i in text_file[0:10] if "end_date" in i][0]\
                        .split(" ")[1].replace("-", "/")
        # Find index where data start
        index = [i+1 for i, v in enumerate(text_file[0:10]) if "#" in v][0]     
    ...
    def runPyRAMID(self, RiverwarePath, BatchFileName, ExecuteNow = True, Log = True):
        '''
        Execute PyRAMID through python code. First, the program will create 
    ...
    def ObjSlotName2FileName(self, Name):
        '''
        Convert ObjSlot name or a list of names from RW to the filename, which 
        RW replace the space and ":" to "_".
        '''
